...
Python中的关系运算符和Java中的相同，包括`>`、`<`、`==`、`>=`、`<=`、`!=`，可以用于相同数据类型之间的比较（不同数据类型之间只有判断相等或不等）。数值型数据之间的比较逻辑很简单（但注意1 == 1.0这样的结果为True），字符型数据之间的比较采用字典序（即依次按照各位置上字符的ASCII码排序，例如'a'<'ab'<'b'）。值得一提的是Python中没有强制转型想要实现ASCII码和单字符串之间的转换需要借助chr(integer)函数和ord(str)函数。另外还有适用于判断是否为字符串的子字符串或列表/元组的子元素的`in`和`not in`运算符。

关系运算符可以用于元组或列表当中，同样地使用类似字典序的逻辑，逐个比较对应位置的元素直到不相同或不能比较为止。

使用myList.sort()方法可以对myList按照自然序进行排序，这一点类似于JDK1.8后List接口新增的sort()方法。

二、逻辑运算符：

关系运算会产生bool型的结果，非真既假。真值之间可以进行逻辑运算，包括与或非（还有同或异或）在Java中我们可以通过`&&`、`||`和`!`来作为这三种运算符，Python中使用`and`、`or`和`not`来作为这三种运算符。Python也默认使用短路求值。

字符串对象提供了一些方法用于对某些特性作出判断：
* isdigit()，字符串是否代表了一个纯数字
* isalpha()，字符串中是否所有字符都是字母表上的字母
* isalnum()，所有字符是否都是字母或数字
* islower()/isupper()，是否都是小写字母/都是大写字母
* isspace()，是否仅含有空白字符
* startswith()/endswith("str")，字符是否以"str"开头或结尾
